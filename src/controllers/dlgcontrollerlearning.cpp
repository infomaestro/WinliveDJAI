...
DlgControllerLearning::DlgControllerLearning(QWidget* parent,
    ...
    QString helpTitle(tr("Click anywhere in Winlive Dj Ai or choose a control to learn"));
    QString helpBody(tr("You can click on any button, slider, or knob in Winlive Dj Ai "
    ...
    QString nextInstructionBody(tr(
            "If you manipulate the control, you should see the Winlive Dj Ai user interface "
    ...
    if (!ControlObject::exists(control)) {
        QMessageBox msg(QMessageBox::Warning,
                ...
                tr("The selected control does not exist.<br>"
                   "This likely a bug. Please report it on the Winlive Dj Ai bug "
                   "tracker.<br>"
...
void DlgControllerLearning::controlClicked(const ConfigKey& controlKey) {
    if (!m_pControlPickerMenu->controlExists(controlKey)) {
        qWarning() << "Winlive Dj Ai UI element clicked for which there is no "
                      "learnable control "
                   << controlKey.group << " " << controlKey.item;
        QMessageBox::warning(
                this,
                VersionStore::applicationName(),
                tr("The control you clicked in Winlive Dj Ai is not learnable.\n"
